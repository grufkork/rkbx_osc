...
use rosc::{OscMessage, OscPacket, encoder::encode};
...
impl Osc{
    pub fn create(conf: ModuleConfig) -> Box<dyn OutputModule>{
        let socket = UdpSocket::bind(conf.get("source_address").unwrap_or(&"127.0.0.1:8888".to_string())).unwrap();
        socket.connect(conf.get("destination").unwrap_or(&String::from("127.0.0.1:9999"))).unwrap();
...
impl OutputModule for Osc{
    ...
    fn beat_update(&mut self, beat: f32){
        ...
        self.send_float("/beat/fraction", beat);
    }
}
